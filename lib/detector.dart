...
class Detector {
  ...
  static Future<Detector> create({
    required String modelPath,
    required String labelsPath,
    required DetectorListener listener,
    ...
    await detector._initialize(modelPath, labelsPath);
    return detector;
  }
  ...
  Future<void> _initialize(String modelPath, String labelsPath) async {
    try {
      ...
      if (inputShape.length > 3 && inputShape[1] == 3) {
      ...
      _isReady = true;
      print('Detector初期化完了: ${tensorWidth}x${tensorHeight}, チャンネル: $numChannels, 要素: $numElements');
    } catch (e) {
      print('Detector初期化エラー: $e');
      _isReady = false;
    }
  }
  ...
  Future<void> detectWithImageProvider(ImageProvider imageProvider) async {
    ...
    try {
      // ImageProviderを画像に変換
      final image = await _imageProviderToImage(imageProvider);
      return detect(image);
    } catch (e) {
      print('画像検出エラー: $e');
    }
  ...
  Float32List _imageToTensor(img.Image image) {
    ...
    for (var y = 0; y < image.height; y++) {
      for (var x = 0; x < image.width; x++) {
        ...
        tensor[i++] = pixel.r / 255.0; // Red
        tensor[i++] = pixel.g / 255.0; // Green
        tensor[i++] = pixel.b / 255.0; // Blue
      }
    }
    return tensor;
  }
  ...
  DetectionList _tensorToResults(Float32List t) {
    ...
    for (var e = 0; e < numElements; e++) {
      ...
      if (maxClass == -1 || maxClass >= _labels.length) {
        continue;
      }
...
class DetectionList {
  ...
  double _iou(Rect a, Rect b) {
    ...
    var unionArea = a.width * a.height + b.width * b.height - intersection.width * intersection.height;
